...
1. **Data Generation:** Input data X ∈ ℝ⁴ is sampled from a normal distribution N(0, 1).
2. **Classical Mapping:** A 3-layer MLP processes the input to produce rotation angles θ.
3. **Quantum Embedding:** The PQC applies RX(θ) and RY(θ) gates followed by a CNOT entangling chain to create the quantum state |ψ(θ)⟩.
4. **Reconstruction:** The model measures the expectation value of the Pauli-Z operator:

   ⟨Zᵢ⟩ = ⟨ψ(θ) | Zᵢ | ψ(θ)⟩

   The training objective is to minimize the **Mean Squared Error (MSE)** between the input X and the quantum measurements X̂:
